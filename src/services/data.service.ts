...
export class DataService {
  ...
  commandLog = signal<string[]>([]);
  
  motorStatus = computed(() => ({
    heater: this.relay1State(),
    motorA: {
      speed: this.grinderSpeed(),
      direction: this.grinderDirection() === 'F' ? 'forward' : 'backward'
    },
    motorB: {
      speed: this.agitatorSpeed(),
      direction: this.agitatorDirection() === 'F' ? 'forward' : 'backward'
    }
  }));
  ...
  addDeficiency(deficiency: string) {
    this.detectedDeficiencies.update(defs => [...defs, deficiency]);
  }
  ...
  simulateConnection() {
    ...
    this.logCommand('DEMO MODE: Connection Simulated');
  }
  ...
  async readNPK(): Promise<void> {
    if (this.isNpkConnected() && this.npkWriter) {
      ...
      try {
        ...
        this.logCommand('Sent to NPK: READ_NPK');
      } catch (e) {
  ...
  async connectNpk() {
    ...
    try {
      ...
      this.logCommand('NPK Serial Connected (9600)');
  ...
  async connectTank() {
    ...
    try {
      ...
      this.isDemoMode.set(false);
      this.logCommand('Tank Serial Connected (9600)');
  ...
  async setDirection(motor: 'A' | 'B' | 2 | 3, direction: 'forward' | 'backward' | 'F' | 'B') {
    const id = (motor === 'A' || motor === 2) ? 2 : 3;
    const dir = (direction === 'forward' || direction === 'F') ? 'F' : 'B';
    ...
    this.logCommand(`Prepared: ${name} Direction -> ${dir}`);
  ...
  toggleHeater() {
    this.toggleRelay(1, !this.relay1State());
  }
  ...
  async setMotorSpeed(motor: 'A' | 'B' | 2 | 3, speed: number) {
    const id = (motor === 'A' || motor === 2) ? 2 : 3;
    ...
    if (isRunning) {
       ...
       this.logCommand(`Prepared: ${name} Speed -> ${safeSpeed} (Wait for Start)`);
    }
  }
  
  private logCommand(msg: string) {
    this.lastCommand.set(msg);
    this.commandLog.update(logs => [msg, ...logs].slice(0, 50));
  }

  private async sendRelayCommand(id: number, state: boolean) {
      const command = `RELAY:${state ? 1 : 0}\n`;
      this.logCommand(`Sent Tank: ${command.trim()}`);
  ...
  async getWeather(lat: number, lon: number) {
    try {
      const response = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_sum&timezone=auto&forecast_days=16`
      );
  ...
  async getCropSuggestions(location: string, landType: string, soilType: string, water: string) {
    try {
      const ai = new GoogleGenAI({ apiKey: GEMINI_API_KEY });
      
      const prompt = `
        Act as an expert agricultural consultant for South India. 
        Based on the following conditions, suggest the best seasonal crops.
