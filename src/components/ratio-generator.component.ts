...
@Component({
  ...
  template: `
    <div class="max-w-5xl mx-auto">
      <header class="mb-8">
        <h2 class="text-3xl font-bold text-[#6A5A4F] mb-2">{{ 'ratio_title' | translate }}</h2>
        <p class="text-stone-600">{{ 'ratio_subtitle' | translate }}</p>
      </header>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-stone-200 h-fit">
          <h3 class="text-lg font-bold text-stone-700 mb-4 border-b pb-2">{{ 'configuration' | translate }}</h3>
          ...
          @if (dataService.selectedCrop()) {
            <div class="mb-6 bg-[#A47E3B]/10 border border-[#A47E3B]/20 p-4 rounded-xl flex items-start justify-between">
              <div>
                <span class="text-xs font-bold text-[#A47E3B] uppercase tracking-wider">{{ 'target_crop' | translate }}</span>
          ...
          @if (dataService.detectedDeficiencies().length > 0) {
            <div class="mb-6 bg-red-50 border border-red-200 p-4 rounded-xl">
               <div class="flex items-center gap-2 mb-2">
                 ...
                 <span class="font-bold text-red-800 text-sm">{{ 'ai_diagnosis' | translate }}</span>
               </div>
               ...
               <p class="text-xs text-red-700 mt-2">{{ 'recipe_adjusted' | translate }}</p>
            </div>
          }

          <div class="mb-6">
            <label class="block text-sm font-medium text-stone-600 mb-2">{{ 'target_volume' | translate }}</label>
          ...
          <div class="bg-stone-50 p-4 rounded-lg border border-stone-100">
            <h4 class="text-sm font-semibold text-stone-500 uppercase mb-3">{{ 'current_levels' | translate }}</h4>
            <div class="grid grid-cols-3 gap-4">
              <div class="flex flex-col gap-1">
                <label class="text-xs font-bold text-blue-600 text-center">{{ 'nitrogen' | translate }} (N)</label>
                <input 
                  type="number" 
                  [value]="inputN()" 
                  (input)="updateN($event)"
              ...
              <div class="flex flex-col gap-1">
                <label class="text-xs font-bold text-orange-600 text-center">{{ 'phosphorus' | translate }} (P)</label>
                <input 
                  type="number" 
                  [value]="inputP()" 
                  (input)="updateP($event)"
              ...
              <div class="flex flex-col gap-1">
                <label class="text-xs font-bold text-red-600 text-center">{{ 'potassium' | translate }} (K)</label>
                <input 
                  type="number" 
                  [value]="inputK()" 
                  (input)="updateK($event)"
            ...
            <p class="text-xs text-stone-400 mt-2 italic text-center">{{ 'auto_sync' | translate }}</p>
          </div>
        </div>
        ...
        <div class="bg-[#FDFBF5] rounded-2xl border-2 border-[#A47E3B] relative overflow-hidden flex flex-col">
          <div class="bg-[#A47E3B] px-6 py-4 flex justify-between items-center">
            <h3 class="text-white font-bold text-lg">{{ 'organic_mix' | translate }}</h3>
            <span class="bg-white/20 text-white text-xs font-bold px-2 py-1 rounded">
               For {{ dataService.selectedCrop() ? dataService.selectedCrop()?.name : ('general_growth' | translate) }}
            </span>
          </div>
          ...
          @if (recommendation()) {
            <div class="p-6 flex-1 flex flex-col">
               ...
               <div class="text-center mb-6">
                 ...
                 <div class="text-stone-500 text-sm">{{ 'total_yield' | translate }}</div>
               </div>
               ...
               <div class="space-y-4">
                 ...
                 <div class="bg-white p-4 rounded-xl shadow-sm border-l-4 border-amber-600">
                    <div class="flex justify-between items-start mb-1">
                      <h4 class="font-bold text-stone-700">1. {{ 'organic_base' | translate }}</h4>
                      <span class="font-bold text-xl text-amber-700">{{ recommendation()?.compost?.amount }} <small class="text-xs">kg/L</small></span>
                    </div>
                    <p class="text-stone-600 text-sm">{{ recommendation()?.compost?.name | translate }}</p>
                    <p class="text-xs text-stone-400 mt-1 italic">{{ recommendation()?.compost?.reason | translate: recommendation()?.compost?.reasonParams }}</p>
                 </div>
                 ...
                 <div class="bg-white p-4 rounded-xl shadow-sm border-l-4 border-red-500">
                    <div class="flex justify-between items-start mb-1">
                       <div class="flex items-center gap-2">
                         <h4 class="font-bold text-stone-700">2. {{ 'nutrient_booster' | translate }}</h4>
                         <span class="bg-red-100 text-red-600 text-[10px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wide">{{ 'essential' | translate }}</span>
                       </div>
                       <span class="font-bold text-xl text-red-600">{{ recommendation()?.booster?.amount }} <small class="text-xs">kg/L</small></span>
                    </div>
                    <p class="text-stone-800 font-bold text-sm">{{ recommendation()?.booster?.name | translate }}</p>
                    <p class="text-xs text-red-500 mt-1 font-medium">{{ recommendation()?.booster?.reason | translate: recommendation()?.booster?.reasonParams }}</p>
                 </div>
      ...
      <div class="mt-8 bg-white p-6 rounded-xl border border-stone-100">
        <h4 class="font-bold text-stone-800 mb-2">{{ 'why_mix' | translate }}</h4>
        <p class="text-stone-600 text-sm leading-relaxed">
           {{ 'formula_desc' | translate: { compost: recommendation()?.compost?.percent, booster: recommendation()?.booster?.percent } }}
           @if (dataService.detectedDeficiencies().length > 0) {
             {{ 'booster_ai_desc' | translate: { deficiency: dataService.detectedDeficiencies()[0] } }}
           } @else {
             {{ 'booster_crop_desc' | translate: { crop: (dataService.selectedCrop() ? dataService.selectedCrop()?.name : ('general_crops' | translate)) } }}
           }
        </p>
      </div>
    </div>
...
export class RatioGeneratorComponent {
  ...
  recommendation = computed(() => {
    ...
    if (aiDeficiencies.length > 0) {
      ...
      if (boosterChoice) {
        boosterReason = 'booster_ai_reason';
        boosterReasonParams = { deficiency: def };
      }
    }
    ...
    if (!boosterChoice) {
       ...
       if (crop) {
         ...
         boosterReason = 'booster_crop_reason';
         boosterReasonParams = { nutrient: targetNutrient, crop: crop.name };
       } else {
         ...
         boosterReason = 'booster_limiting_reason';
         boosterReasonParams = { nutrient: targetNutrient };
       }
    ...
    if (!boosterChoice) {
      ...
      boosterReason = 'booster_general_reason';
    }

    const boosterItem: RecipeItem = {
      name: boosterChoice?.name || 'general_booster',
      amount: amountBooster,
      reason: boosterReason,
      reasonParams: boosterReasonParams,
      color: 'red',
