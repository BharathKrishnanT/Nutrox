  ...
  template: `
    <div class="h-full">
      <div class="bg-gradient-to-r from-teal-800 to-teal-700 p-8 rounded-3xl shadow-lg text-white mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 class="text-3xl font-bold mb-2">{{ 'tank_title' | translate }}</h2>
          <p class="opacity-80">{{ 'tank_subtitle' | translate }}</p>
          ...
          <div class="mt-4 flex gap-2">
            @if (dataService.isTankConnected()) {
              <div class="bg-white/20 backdrop-blur-md px-3 py-1 rounded-full text-sm font-semibold flex items-center gap-2 border border-white/10">
                <span class="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span> {{ 'tank_usb_connected' | translate }}
              </div>
            } @else if (dataService.isDemoMode()) {
              <div class="bg-white/20 backdrop-blur-md px-3 py-1 rounded-full text-sm font-semibold flex items-center gap-2 border border-white/10">
                <span class="w-2 h-2 bg-yellow-400 rounded-full"></span> {{ 'simulated_data' | translate }}
              </div>
            } @else {
              <div class="bg-white/10 backdrop-blur-md px-3 py-1 rounded-full text-sm font-semibold border border-white/10">
                 {{ 'system_disconnected' | translate }}
              </div>
            }
          </div>
        </div>
          ...
          @if (!dataService.isTankConnected()) {
             <div class="flex gap-2">
               ...
               <button (click)="connectUsb()" class="bg-white text-teal-800 hover:bg-teal-50 px-6 py-2 rounded-xl font-bold transition-colors flex items-center gap-2 shadow-lg">
                 <span>🔌</span> {{ 'connect_tank' | translate }}
               </button>
             </div>
          } @else {
             <div class="text-right opacity-70 text-sm">
               {{ 'reading_tank_stream' | translate }}<br>
               Baud: 9600
             </div>
          }
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-stone-100 flex flex-col items-center justify-center min-h-[160px]">
          <div class="text-stone-400 font-medium uppercase tracking-wider text-xs mb-2">{{ 'methane' | translate }}</div>
        ...
        <!-- Temperature -->
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-stone-100 flex flex-col items-center justify-center min-h-[160px]">
          <div class="text-orange-400 font-medium uppercase tracking-wider text-xs mb-2">{{ 'temperature' | translate }}</div>
        ...
        <!-- Humidity -->
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-stone-100 flex flex-col items-center justify-center min-h-[160px]">
          <div class="text-blue-400 font-medium uppercase tracking-wider text-xs mb-2">{{ 'humidity' | translate }}</div>
        ...
        <!-- pH Level -->
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-stone-100 flex flex-col items-center justify-center min-h-[160px]">
          <div class="text-purple-400 font-medium uppercase tracking-wider text-xs mb-2">{{ 'ph_level' | translate }}</div>
          ...
          <div class="mt-2 text-xs font-medium text-purple-500 uppercase tracking-wide">
            {{ getPhStatus(dataService.tankReadings().ph) | translate }}
          </div>
        </div>
      ...
      <div class="mt-6 bg-white p-6 rounded-2xl border border-stone-200">
        <h3 class="font-bold text-stone-700 mb-2">{{ 'system_health' | translate }}</h3>
        <div class="flex items-center gap-2">
           ...
           <span class="text-sm text-stone-500">
             @if(dataService.isTankConnected()) {
               {{ 'tank_stream_active' | translate }}
             } @else if (dataService.isDemoMode()) {
               {{ 'simulation_mode' | translate }}
             } @else {
               {{ 'waiting_for_connection' | translate }}
             }
           </span>
        </div>
      </div>
